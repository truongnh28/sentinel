...
## Key Findings (tóm tắt định hướng)
...
## Details — Phương pháp luận theo từng Stage

Với **mỗi stage**, cấu trúc gồm: *Mục đích • Giả định • Inputs • Thủ tục (numbered) • Pseudocode/lệnh/schema • Con số stage sinh ra (INPUT/OUTPUT) • Pitfalls & thách thức của hội đồng • Nguồn.*
...
**Inputs.** Không có input đo đạc; đây là stage định nghĩa. Các hằng số `λ_Q, λ_T, η_Q, τ, β, b_0, c (Theorem 4)` được **chọn** ở các stage sau (đánh dấu là "student must choose").
...
| λ_Q, λ_T | trọng số loss | INPUT (student chọn) |
| η_Q | chi phí quarantine | INPUT (student chọn) |
| b_0 | prior | INPUT (student chọn) |

**Pitfalls & thách thức hội đồng.**
...
**Inputs.** Dataset `princeton-nlp/SWE-bench` (full/Verified/Lite) trên Hugging Face; Docker; `swebench` package.
...
**Inputs.** κ đích của bản thảo: **0.4 / 0.9 / 1.6 / 4.1 CPU-minutes** cho (insertion / retrieval / delegation / commit).
...
2. **Depth-dependence.** Học viên phải định nghĩa `κ(k, depth)`, ví dụ `κ(k, depth) = κ_base(k) · (1 + ρ_d·(depth−1))` với ρ_d là hệ số tăng tuyến tính theo độ sâu kiểm tra (audit sâu tốn hơn). Đây là INPUT do student chọn — bản thảo không cho.
3. **Tính χ.** `χ = max_{k,k'}|κ(k)−κ(k')|/κ̄`. Với κ = (0.4,0.9,1.6,4.1): tử số `= 4.1−0.4 = 3.7`.

**Vấn đề κ̄ — bảng so sánh định nghĩa ứng viên (BẮT BUỘC thảo luận):**

| Định nghĩa κ̄ | Giá trị κ̄ | χ = 3.7/κ̄ | Có ra 1.34? |
|---|---|---|---|
| mean(0.4,0.9,1.6,4.1) = 1.75 | 1.75 | **2.11** | Không (số bản thảo tự nhận sai) |
...
| ρ_d (depth coef) | INPUT (student chọn) |

**Pitfalls & thách thức hội đồng.** *"χ=1.34 tính thế nào?"* → nếu không giải thích được sẽ bị bắt lỗi ngay. Xử lý bằng cách nêu rõ mâu thuẫn và công bố công thức của mình. *"CPU-minutes có ổn định giữa các máy?"* → dùng CPU-time + median + pin frequency; báo cáo cấu hình phần cứng.
...
**Inputs.** Δ ∈ {0,1,2,4,8}; ε (cường độ nhiễm) do student chọn; 18 scripted attacker policies (7 held-out) + 3 LLM attackers (báo cáo riêng).
...
| ε | INPUT (student chọn) |
| 18 scripted attackers (7 held-out) | INPUT (thiết kế) |
| 3 LLM attackers | INPUT (báo cáo riêng) |
| 27.6% harm vs held-out | OUTPUT (chạy) |

**Pitfalls & thách thức hội đồng.** *"Δ đo từ git có phải nhân quả không?"* → co-change là *tương quan tiến hoá*, không hẳn nhân quả; nghiên cứu "Is Code Co-Committal an Indicator of Evolutionary Coupling?" (Software 2026, doi:10.3390/software5010011) cảnh báo coupling *"vary widely in both strength and frequency."* → kết hợp với dependency graph + git bisect để tăng độ tin.
...
**Inputs.** 620 matched changes; two-sample test; classifier two-sample test (C2ST) đo bằng AUC.
...
3. **Ý nghĩa AUC ≤ 0.56.** AUC=0.5 là ngẫu nhiên hoàn hảo; 0.56 là "chỉ nhỉnh hơn ngẫu nhiên một chút", tức surface-feature discriminator gần như bất lực — corpus đạt yêu cầu "matched". Kiểm định ý nghĩa: hoán vị nhãn (permutation test) B=1000 lần để lấy phân phối null của AUC; nếu AUC quan sát nằm trong khoảng null → không tách được.
4. **Sample-size.** Với hai lớp mỗi ~310 mẫu (tổng 620), sai số chuẩn của AUC ≈ `sqrt(AUC(1−AUC)/min(n⁺,n⁻))` ≈ `sqrt(0.56·0.44/310) ≈ 0.028`; nghĩa là khoảng tin cậy 95% cho AUC≈0.56 là [0.505, 0.615] — cần n lớn hơn nếu muốn phân biệt chặt 0.56 với 0.5. **Khuyến nghị nêu rõ n để phòng thủ trước hội đồng.**
5. **Two-sample test bổ sung.** MMD hoặc energy distance trên đặc trưng để xác nhận marginal khớp.
6. **Ước lượng β (benign drift rate).** β = tần suất một CarrierSignal tăng do thay đổi lành tính (không độc). Đo: chạy các workflow *sạch* (không attacker) qua particle filter, đếm tỉ lệ carrier có tín hiệu vượt ngưỡng nhưng thực tế benign → β. Đây là INPUT student đo.
...
| β | INPUT (student đo trên workflow sạch) |

**Pitfalls & thách thức hội đồng.** *"AUC 0.56 khác 0.5 có ý nghĩa thống kê không?"* → với n=620, biên sai ~0.028; 0.56 có thể *không* khác 0.5 ở mức α=0.05 — điều này THỰC RA có lợi (càng gần ngẫu nhiên càng tốt), nhưng phải trình bày CI. *"Matching có che giấu tín hiệu thật không?"* → matching chỉ trên *surface features*; tín hiệu ngữ nghĩa sâu vẫn có thể tồn tại (đó là điều detector ở Stage 5 khai thác).
...
**Pitfalls & thách thức hội đồng.** *"Tại sao mean loại được bias mà max không?"* → trình bày E[max]≈√(2 ln n) (đây là câu hỏi hội đồng rất dễ hỏi — phải thuộc). *"ψ,φ có đo từ detector thật không?"* → trong bản thảo là setting giả định; học viên có thể calibrate từ detector thực (ROC) rồi chọn 3 điểm trên đường ROC.
...
- **Belief-threshold** (~10 policy): audit khi `b_t(carrier k) > τ_k`; tham số = ngưỡng τ (student chọn τ).
...
| 28 policy / 3 họ | INPUT (student định nghĩa — bản thảo không cho) |
...
| τ (ngưỡng belief) | INPUT (student chọn) |

**Pitfalls & thách thức hội đồng.** *"2048 particle có đủ không?"* → chạy convergence study (variance của belief theo N=512/1024/2048/4096). *"ρ đo bằng TV trên không gian policy nào?"* → phải cố định discretization belief trước. *"28 policy chọn thế nào?"* → nêu rõ 3 họ + tham số + lý do phủ.
...
### STAGE 7 — Worked Example thủ công (BẰNG TAY)

**Mục đích.** Minh hoạ toàn bộ pipeline trên một game tí hon để học viên tính TAY, phục vụ bảo vệ.

#### 7A — Minimax zero-sum game K=2, H=4 (harm matrix)
...
```
max_{x, v}  v
s.t.  Σ_i x_i · Harm[i][j] ≥ v   ∀ cột j   (định dạng cho attacker-maximiser; đảo dấu cho defender-min)
      Σ_i x_i = 1,  x_i ≥ 0
```
...
```
→ belief giảm 0.10 → 0.0186. **Quyết định quarantine** khi `p_t > τ` (student chọn τ, ví dụ τ=0.8 → chỉ quarantine sau ≥2 alarm).

**Con số stage sinh ra.** Toàn bộ là minh hoạ (không phải số bản thảo): V=0.30, x=0.5, y=1/3, p_1=0.440, p_2=0.848. Đều là OUTPUT của phép tính tay.

**Pitfalls & thách thức hội đồng.** *"Vì sao 2 alarm mới quarantine?"* → phụ thuộc τ và λ_Q (false-quarantine penalty). Nối với Stage 9 (false quarantine 3.2%→5.1%). *"Game 2×2 có đại diện K=4 không?"* → không, chỉ minh hoạ; game thật giải bằng backward induction POMDP.
...
**Con số stage sinh ra.** Không sinh số; sinh *bằng chứng tính toàn vẹn*. Đây là điều kiện tiên quyết để mọi OUTPUT ở Stage 9 đáng tin.

**Pitfalls & thách thức hội đồng.** *"Làm sao biết bạn không tune sau khi thấy kết quả?"* → hash + commit chain + held-out set. Mô hình tham chiếu: "verify_paper_numbers.py re-derives every number… from the raw CSVs" (Zenodo artifacts). *"Held-out có thực sự held-out?"* → hash của 7 attacker phải có trong manifest freeze nhưng kết quả của chúng chỉ tính sau.
...
**Mục đích.** Chạy toàn bộ ma trận thực nghiệm và tính các OUTPUT; tất cả là số "projected" — học viên sẽ ra số khác.
...
**Bảng: Outputs (đều PROJECTED — học viên sẽ khác).**
...
**Theorem 4 (distinguishability budget).** `B ≥ c·κ̄·(1+χ)·log(1/α)/γ² · (1 + (Δ/H)·K)`. Hằng số `c` **student phải chọn/hiệu chuẩn** (fit từ small games: chọn c nhỏ nhất sao cho bound giữ trên 240 small games). Corollary: commit-time audit đủ khi Δ nhỏ hoặc K hiệu dụng =1 → giải thích crossover ở Δ≈2.

**Pitfalls & thách thức hội đồng.** *"Pool hay grid?"* → grid, kèm CI mỗi cell. *"CI có tính tương quan nội cụm?"* → cluster bootstrap; báo cáo SE inflation ratio. *"Ablation 0.264 < 0.272 nghĩa gì?"* → phải điều tra; nếu bỏ benign-drift mà harm giảm thì hoặc là mô hình benign-drift đang đánh đổi harm lấy false-quarantine thấp hơn, hoặc là số projected không tự nhất quán — nêu thẳng.
...
**Pitfalls & thách thức hội đồng.** *"Bạn có double oracle hội tụ không?"* → báo cáo equilibrium gap theo iteration. *"Non-convex thì nghiệm có toàn cục không?"* → dùng FPTAS của Blocki để có bảo đảm ε-tối ưu.
...
## Bảng Parameter Provenance (ánh xạ MỌI con số → stage → INPUT/OUTPUT → tái lập được?)
...
| 28 policy / 3 họ | 6 | INPUT (student định nghĩa) | Phương pháp (bản thảo không cho danh sách) |
...
| c (Theorem 4) | 9/10 | INPUT (student fit) | Phương pháp |
| τ, λ_Q, λ_T, η_Q, b_0 | 0/5/6 | INPUT (student chọn) | Phương pháp |
| depth-dependence κ(k,depth) | 2 | INPUT (student định nghĩa) | Phương pháp |

---

## Recommendations (các bước hành động, có mốc quyết định)
...
4. Đo κ (median CPU-time, ≥30 lần lặp, pin frequency); **công bố công thức κ̄ = arithmetic mean** và chấp nhận χ của bạn ≈ 2.11 (khác 1.34).
5. Xây benign corpus, chạy C2ST, xác nhận AUC gần 0.5 (báo cáo CI ±0.028). Nếu AUC > ~0.6 → matching chưa đủ, tăng n hoặc siết đặc trưng.
...
- **AUC 0.56 với n=620** có thể không khác 0.5 ở α=0.05 (biên ~0.028) — điều này *có lợi* nhưng phải trình bày CI.
