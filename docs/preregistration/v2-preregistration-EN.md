...
- 26/09, the SINGLE eval run of the D35 addendum (`tools/run_d35.py --split eval --jobs 10`), 02:03:10–02:04:17:
  - Header at start and at summary time: `freeze-d35: clean sha256:e46f8a5c2f94  |  base freeze: clean sha256:c789fa7362e0`; git `0dea59d`. The summary's provenance records `git_clean: false`: the only change under `auditgame/` at the start was the run's own log file (`spikes/v2-addendum/d35-log.txt`, created by `tee` just before the tool started); `git status` just before showed one file outside `auditgame/` only. Neither freeze depends on that file.
  - Records: `d35-main` 107,212 lines, `d35-br` 56 lines; sha256 in `auditgame/spikes/v2-addendum/d35-records.sha256` (re-checked: OK). Stdout in `d35-log.txt`.
  - Reproduction (gating): V of B1 and Sentinel-A1 equals the main summary at all 4 ρ. Post-run check (not gating): the `v_br_by_delta` of both equals the main summary's `table2` at all 4 ρ.
  - Predictions scored: P7a, P7b, P7c hold (retained: swap 0.975 / 0.904 / 0.799; down1 0.881 / 0.880 / 0.836; down2 0.548 / 0.394 / −0.005). P8a, P8b hold. P9a, P9c hold; P9b FAILS at ρ = 0.25 (V(B1) − V(fixed) = +0.0129 [−0.1479, 0.1571]); it holds at ρ = 0.5 and 1.
  - Readings: (A) holds; (B) holds at ρ ≤ 0.5; (C) the fixed interleave is worse than Sentinel at all 4 ρ, so at no ρ is the gain "not attributable to randomisation" under D35 (C); the converse is not inferred.
  - Numbers: `auditgame/spikes/v2-addendum/d35-summary.json`.
- 26/09, post hoc #3 (the third critique round; a post hoc note, not a new D line): `auditgame/spikes/v2/posthoc/posthoc3.py` → `posthoc3.json`. It reads only `sweep-eta.jsonl` (main run) and `d35-main.jsonl` (addendum), checks both sha256 values and that both hold the same episodes; no simulation, no tuning.
  - At every η_Q ≥ 0.02 of the grid (mixtures fixed), Sentinel-A1 has both a lower FQ and a lower V than B2 FQ-matched at every ρ ≤ 0.5; every paired 98.75% interval of V(B2 FQ-matched) − V(S) lies above 0 (lowest lower bound 0.0933). At η_Q = 0.01 this holds at ρ ≤ 0.25; at ρ = 0.5 FQ is 2.56 against 2.42.
  - A revealed-draw best response (separating randomisation from per-workflow coverage) cannot be computed from the records: the eval best response stores cross-fitted values per system, not per member. It would need a new eval simulation (another addendum); not done.
