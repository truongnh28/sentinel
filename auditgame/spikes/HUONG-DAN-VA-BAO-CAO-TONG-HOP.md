...
| 2 | **A1b** | `ce55c56` | **Chốt Hai Quyết định Phương pháp luận:** Giữ đặc trưng `topic` trong $F_{\text{match}}$ (báo cáo trung thực hai khâu); xác lập **Họ muối (Salt Family)** với ngưỡng $\ge 80\%$ ($14/17$ muối) là đơn vị lặp lại độc lập thay vì split đơn lẻ. Tạo spike `spikes/cong-v2-chot.md`. | Hoàn tất |
| 3 | **D1** | `4edb4f9` | **Sweep Chấm $L$ trên Payload v2:** Chạy lại toàn bộ lưới $d'$ trên v2 ở 2 chế độ (`pinned` và `following`); tạo spike `spikes/sweep-loss-v2.md`; xác nhận phán quyết Khung B (Sentinel thua B5 trên $L$). | Hoàn tất |
...
| 5 | **A3** | `1392edc` | **Đường Cong Dải Nhiễu theo Quy mô $n$:** Tạo module `analysis/band_vs_n.py` và test `test_band_vs_n.py`; đo thực nghiệm trên $n \in \{100, 200, 400, 900, 1800\}$ qua họ muối; tạo spike `spikes/dai-theo-n.md`. | Hoàn tất |
| 6 | **P1–P4**| `cb48e90` | **Thí nghiệm P2 Ba Arm & Đọc Mô Hình Tiếp Nhận:** Tạo bộ điều phối `spikes/p2_arms.py`, kiểm thử `test_p2_arms.py`, ghi nhận spike `spikes/p2-three-arms.md`; xác nhận **Giả thuyết (ii)**: Agent từ chối làm theo `raw_write`, chứng minh mọi số harm trong bài là harm dưới MockAgent. | Hoàn tất |
...
```bash
cd /Users/truong.nh/Me/obsidian/HCMUT/code/Sentinel/auditgame
...
Sau khi sinh ra các file `.json`, chạy script sau để tự động tổng hợp số liệu vào [spikes/sweep-loss-v2.md](file:///Users/truong.nh/Me/obsidian/HCMUT/code/Sentinel/auditgame/spikes/sweep-loss-v2.md):
...
```
Kết quả được lưu vào `spikes/band_curve.json` và phân tích trong `spikes/dai-theo-n.md`.
