...
| **Kiểm thử** | `tests/run_all.py --all` ngày **24/09**: **811 đạt · 2 không đạt · 0 bỏ qua** (813 test) khi máy rảnh. Cổng 2 đỏ 203/205 (hai lỗi cũ), cổng 3 xanh 15/15. ⚠️ **Cổng 1 chập chờn**: chạy riêng thì 29/29 test container đạt, nhưng một lượt full lúc máy đang tải nặng lại bỏ qua đủ 13 test — nghi phép kiểm container quá hạn khi tranh CPU, **chưa xác nhận** ([§3.5](#35-kiểm-thử)). |
...
1. **95% là công đã bỏ ra, không phải bài đã viết.** Câu trả lời cho "bài viết được bao nhiêu" là **35–40%** sau khi có mục Evaluation, không phải 95%. Cỗ máy đo nay chạy đúng thang, đúng giá và đúng attacker — và nó cho ra **một kết quả dương có phạm vi**: phân bổ audit đáng giá trong dải ngân sách 0,5–0,75 (+41,6% … +50,7%), vô nghĩa ở hai đầu. Ở đúng mức ngân sách cũ thì không ô nào vượt 15%.
...
| **Còn nợ** | Hằng số $c$ (chờ quyết định mô hình quan sát) và 240 game nhỏ. $\Delta$ thực và $\rho$ **đã xong**. Nợ thật bây giờ là **bốn mục bài chưa viết** |
...
| Cổng | Kết quả |
|---|---|
| Gate 1 — Integrity | **593/593 khi máy rảnh.** Nhưng ⚠️ **không ổn định**: một lượt full lúc CPU đang tải nặng bỏ qua đủ 13 test container, trong khi chạy riêng cùng lúc đó thì 29/29 đạt. Nghi `container_ready()` quá hạn khi tranh tài nguyên — **phải xác nhận trước khi trích 593/593 vào bài** |
