...
| 3 | 🟡 Một phần | Sinh $(\iota,\sigma)$ có lọc nổ sớm; **18 attacker kịch bản** (6 luật đọc workflow thêm 23/09); chia held-out bằng hash; **marker duy nhất mỗi cấu hình** (`build.marker_for`); **23/09: đo $\Delta$ thực trên 10 repo SWE-bench** (SZZ-lite) | $\Delta$ theo task: trung vị **0** ở 8/10 repo, django **4,5**; chỉ **19%** instance có $\Delta \ge 2$. $\Delta$ theo commit: trung vị 14–194, p75 tới 935 | Số dogfood cũ (trung vị 3) đo trên repo đề tài, **không** phải quần thể mục tiêu; đơn vị task phụ thuộc mật độ lấy mẫu instance nên phải báo theo repo. Vùng $\Delta \ge 2$ chỉ phủ ~1/5 workload ⇒ ranh giới vùng nên phát biểu theo tỉ lệ workload từng repo. Còn lại: held-out **9/18** (theo hash, không vẽ lại để khớp 7/18 của draft); best-response mới là max trên $(\Delta,k)$ (`runner.best_response_gap`), chưa tối ưu tự do trên $(k,\iota,\sigma)$; chưa có $\Delta = 8$. Harm **đã validate trên tập con**: 15 instance, test ẩn viết tay, khớp 30/30 trong miền bộ mô phỏng biểu diễn được và 0/30 ngoài miền đó |
...
`tests/run_all.py --all` chạy lại cuối ngày **23/09/2026**: **795 đạt · 2 không đạt · 13 bỏ qua** (810 test). Hai lỗi là **đúng hai lỗi cũ** — không hồi quy nào qua toàn bộ đợt sửa hôm nay.
