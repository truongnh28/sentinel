...
| **Khung bài** | **A′ — "bản đồ chế độ"** (chốt 18/09) — nhưng bằng chứng hai ngày qua **nghiêng hẳn về khung B** (benchmark tự phản biện), xem bốn kết quả âm ở [§Được bao nhiêu phần trăm](#được-bao-nhiêu-phần-trăm) và issue [#13](#34-issue-cần-xử-lý). Chưa đổi, vì đó là quyết định của thầy và trò chứ không phải của một phép đo. **24/09:** lưới chính chưa bao giờ gọi `costs.install()` nên chạy trên bảng placeholder, và đường cách ly hệ quả-của-audit **không bị trừ ngân sách**. Sửa cả hai thì khoảng cách dẫn của `Sentinel` trước B1 ở $\Delta = 4$ tụt **+43,4% → +3,5%** — nó được mua bằng cách ly miễn phí. Cái **sống sót** là họ SSG — nhưng **không đủ**: ở $N = 100$ kèm CI95, `SSG-full receding` chỉ đạt **+14,5%** CI [+9,7; +19,7] ở ô tốt nhất, và **không ô nào vượt ngưỡng 15%** ở bất kỳ $\Delta$ nào. SSG-G vẫn tốt nhất trên cả hai trục ($L = 0{,}988$, regret $= 0{,}0119$) vì nó **không bao giờ cách ly** nên không có gì để mất. |
...
1. **90% là công đã bỏ ra, không phải bài đã viết.** Cỗ máy đo nay chạy đúng thang và đúng giá, nhưng chính vì thế mà *số để đưa vào bài* vừa xấu đi: ở $N = 100$ kèm CI95, **không ô nào** vượt ngưỡng 15% ở bất kỳ $\Delta$ nào. Câu trả lời cho "bài viết được bao nhiêu" là **25–30%**, không phải 90%.
...
*Về chính luận điểm — bốn kết quả âm, và chúng là phần trung thực nhất của bài:*
...
Bốn kết quả sau đều **bác bỏ dự phóng của draft**, và ba trong bốn chỉ lộ ra khi một hằng số đặt tay bị thay bằng số đo. Đó là lập luận cho **khung B** (benchmark tự phản biện), không phải cho A′.
...
W1–W5 xong 23/09; W7 ($\rho$) xong sau khi có thư viện 28 policy ($\rho = 0{,}5078$, cận Mệnh đề 6 **rỗng**); $\kappa_{\text{commit}}$, $\lambda_Q/\lambda_T$ và quy ước quét $\chi$ đều đã chốt. **Chỉ W6 ($c$) còn hỏng tiền đề**, và nó chờ quyết định T1 về mô hình quan sát chứ không chờ một phép đo. Nói cách khác: **phần đo đã cạn** — cái còn lại ở [§3.4](#34-issue-cần-xử-lý) là quyết định khung bài, không phải số.
