...
| **G. Tính toàn vẹn** | **100%** (5/5) | ✅ Ba cổng kiểm thử, 813 test · ✅ Tài liệu tiền đăng ký · ✅ Digest rời **đã gộp vào một ô** (`freeze.pins`): cổng v2 · bank payload · prompt judge · luật subset, mỗi pin hai cột *live* và *declared* · ✅ Manifest đóng băng toàn phần (`freeze.py`: digest mã nguồn — nay gồm cả **đường chấm điểm test ẩn** — + hằng đọc từ module sống + hai registry + bảng sinh + bốn pin) · ✅ Harness từ chối policy chưa đóng băng (`freeze.require_frozen`, gọi trong `experiment.py`) |
| **H. Tài liệu** | **88%** (3,5/4) | ✅ Phương pháp luận · ✅ Parameter Estimation Report · ✅ Reports và báo cáo tiến độ · 🟡 Bản thảo ACM: **Evaluation** ([docs/manuscript/Evaluation.md](docs/manuscript/Evaluation.md), kèm Hình 1 sinh từ dữ liệu) và **Related Work** ([docs/manuscript/Related-Work.md](docs/manuscript/Related-Work.md), §7.7 liệt kê 5 lỗi trích dẫn trong chính danh mục của ta) đã có bản nháp; còn Intro, Model, Design |
...
| $\rho$ | 0,07 ⇒ cận $H\rho\,\mathrm{range}(L) = 0{,}42$ | **0,7500** so với nghiệm tối ưu thật (240 game nhỏ giải chính xác) | Lớn hơn **10,7×** ⇒ cận thành **4,06**, lớn hơn toàn miền giá trị của $L$, tức **rỗng**. Và tệ hơn rỗng: lớp tối ưu **chính xác ở 175/240 game** ⇒ TV **không theo dõi mất mát** |
