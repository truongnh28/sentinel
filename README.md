...
| **Kiểm thử** | `tests/run_all.py --all` cuối ngày **24/09**: **838 đạt · 2 không đạt · 0 bỏ qua** (840 test). Cổng 1 **620/620**, không còn test container nào bị bỏ qua (issue #9 đã sửa). Cổng 2 đỏ 203/205 (hai lỗi cũ), cổng 3 xanh 15/15 ([§3.5](#35-kiểm-thử)). |
...
| **G. Tính toàn vẹn** | **100%** (5/5) | ✅ Ba cổng kiểm thử, 840 test · ✅ Tài liệu tiền đăng ký · ✅ Digest rời **đã gộp vào một ô** (`freeze.pins`): cổng v2 · bank payload · prompt judge · luật subset, mỗi pin hai cột *live* và *declared* · ✅ Manifest đóng băng toàn phần (`freeze.py`: digest mã nguồn — nay gồm cả **đường chấm điểm test ẩn** — + hằng đọc từ module sống + hai registry + bảng sinh + bốn pin) · ✅ Harness từ chối policy chưa đóng băng (`freeze.require_frozen`, gọi trong `experiment.py`) |
...
**9 hoàn thành · 1 làm theo hướng khác · 1 chờ mốc · 0 chưa bắt đầu** (11 stage, 0–10). Rà soát ngày 22/09 bằng cách đọc mã và chạy lại toàn bộ test; cập nhật 23/09 sau đợt đo W1–W6, và **24/09 sau khi sửa thang chi phí, giá cách ly, ngân sách theo chân trời, rồi rà lại toàn repo** ([runbook](docs/AuditGame-SE_Parameter_Measurement_Runbook.md)). Đợt rà cuối ngày 24/09 đưa **Stage 7 lên hoàn thành** (hai ví dụ tay thành test) và đổi *nội dung* của Stage 2, 3, 6 và 9 — chi tiết trong cột Issue.
...
- **Việc mở** — một bước của thủ tục chưa làm. Còn đúng một: Stage 8, `freeze.write()`, hẹn **29/09**. (Bước 8 của Stage 9, hằng $c$, đã đóng 24/09 dưới dạng *không áp dụng, có lý do đo được*.)
...
| 5 | ✅ **Xong** — `freeze.py` + `require_frozen` nối vào `experiment.py`; **24/09 gộp nốt bốn digest rời** vào `freeze.pins` | Đủ cơ chế để chứng minh "không tune sau khi thấy kết quả". Trước đó ba cấu hình được ghim bằng ba digest riêng, mỗi cái chỉ thấy tệp của chính nó: `freeze: clean` in ra được trên lượt chạy mà tiêu chí cổng 2, bank payload hoặc prompt judge đã đổi. Digest rời **không tổ hợp lại được** | Còn **chạy** `freeze.write()` vào mốc 29/09; trước đó `require_frozen` là no-op có chủ ý. Prompt judge và luật subset vẫn **chưa có bản khai** để đối chiếu — cột `declared` là `null`, nghĩa là pin bắt được *đổi kể từ lúc đóng băng* nhưng không bắt được *lệch với artefact* |
...
`tests/run_all.py --all` chạy lại cuối ngày **24/09/2026**, lúc máy rảnh và không sửa file nào trong lúc chạy: **838 đạt · 2 không đạt · 0 bỏ qua** (840 test). Hai lỗi là **đúng hai lỗi cũ** — không hồi quy nào qua bốn đợt sửa của hai ngày (thang chi phí, giá cách ly, ngân sách theo chân trời, cờ $\Delta$). Cổng 1 bắt được một lỗi thật giữa đường: `spent` của `replay.rescore` lệch với chạy trực tiếp vì tôi mới tính tiền cách ly ở một trong hai đường — đúng việc test I9 sinh ra để làm.

| Cổng | Kết quả |
|---|---|
| Gate 1 — Integrity | **620/620**, **0 bỏ qua**. Issue #9 đã sửa: 13 test container từng bị bỏ qua vì `docker image inspect` trên Docker Desktop trả "No such image" theo từng đợt, chứ **không phải** do CPU tải nặng như chẩn đoán cũ |
