...
| **Rủi ro lớn nhất** | (1) **Chỉ 2/12 ô vượt ngưỡng 15%** của chính dự án, cả hai là `SSG-full receding` ở $\Delta = 4$. `Sentinel` coi như hoà B1 (+1,7% … +4,6%) ⇒ bài phải đổi chính sách chủ đạo, hoặc phát biểu lại luận điểm. (2) Agent thật làm theo payload **0/7** và detector thật chỉ đạt $d' \le 0{,}50$ so với $d'^{*} = 2{,}85$ cần có ⇒ mọi số harm là harm dưới mô hình tiếp nhận của MockAgent. (3) Ô $\Delta = 4$ của cổng v2 vẫn đỏ — permutation test xác nhận độc lập là **tách được thật** ($p = 0{,}010$). (4) Lưới chính vẫn 40 workflow, chưa có $\Delta = 8$. (5) $\lambda_T = 0{,}5$ nằm gần mép trên vùng SSG-G thắng; khai 0,6 là B1 thắng. |
...
1. **Nhóm A và D gần xong, nhóm F mới 31%.** Cỗ máy đo đã chạy được, nhưng phần lớn *số để đưa vào bài* thì chưa có. Câu trả lời cho "bài viết được bao nhiêu" là **25–30%**, không phải 85%.
2. **Nút thắt đã chuyển.** Thư viện 28 policy và manifest đóng băng đã xong; cái còn kéo tụt là **6 attacker còn thiếu** (nhóm B) và **lưới đầy đủ** (nhóm F: $\Delta = 8$, $N = 100$, ablation rotation + benign-drift). Exploitability và worst-case held-out đều đã có số — và **cả hai ngược dấu dự phóng**. Cả ba nằm trong [§3.4](#34-issue-cần-xử-lý) và [§4](#4-kế-hoạch-bổ-sung-đưa-các-hạng-mục-còn-thiếu-vào-sprint).
