...
2. **Nút thắt là viết, không phải đo.** Phần đo đã cạn: thư viện 28 policy, manifest đóng băng, 18/18 attacker, $\Delta = 8$, $N = 100$ kèm CI, ablation đủ bốn arm, attacker best-response, quét ngân sách — đều xong. Còn **ba mục bài** chưa viết (Intro, Model, Design) và **một quyết định** phải chốt với thầy: đổi chính sách chủ đạo sang họ SSG và đổi trục chế độ sang ngân sách (issue #13). Xem thêm #15 ở [§3.4](#34-issue-cần-xử-lý).
...
| **12 — T3 29/09** | ✅ **Xong sớm 24/09.** **Đóng băng số**: manifest hash, harness từ chối policy chưa đóng băng; chạy lại lưới trên cấu hình đã đóng băng | Chạy ba cổng, bật Docker cho 13 test của cổng 1 | Ba cổng xanh, hoặc đỏ có giải trình; số cuối cùng |
...
Các số "đo được" dưới đây chạy trên **thiết lập hiện tại**: MockAgent, thang USD, cách ly có tính tiền, thư viện 28 policy, held-out 9/18 theo hash. Chúng chưa phải phép tái lập cuối cùng: chưa đóng băng, và issue #13 còn mở.
...
|---|---|---|---|
| 1 | 🔴 Ô $\Delta = 4$ của cổng v2 đỏ — và **24/09 nó trở thành issue chặn**: hai ô duy nhất vượt ngưỡng 15% đều nằm ở $\Delta = 4$, nên giao giữa "vượt ngưỡng" và "được cổng chứng nhận" là **rỗng** (vùng chứng nhận tốt nhất chỉ +10,6%, `spikes/certified-power.json`) | Quyết định khung bài. **Permutation test xác nhận độc lập**: $\Delta=4$ tách được thật ($p = 0{,}010$), không phải giả định của cận tham số | Chốt trước: chứng nhận $\Delta \in \{0,2\}$ hay lùi khung B. Hai phép kiểm **bất đồng** ở $\Delta \in \{0,2\}$ — cận Hanley–McNeil nói vượt trần, hoán vị nói bình thường ($p = 0{,}28$/$0{,}25$) ⇒ cận tham số đang **quá bảo thủ** ở đây |
...
| 9 | ✅ **Xong** — gate 1 **624/624**, 0 bỏ qua | **Chẩn đoán cũ sai.** Không phải CPU tải nặng, không phải hết giờ: Docker Desktop 29.5 trả "No such image" cho `inspect` theo tag **theo từng đợt vài giây** trong khi `docker image ls` vẫn liệt kê image; `container_ready()` đọc mọi mã khác 0 thành "chưa build" rồi skip. Ba lượt full ngày 24/09 đều dính, kể cả lượt lúc rảnh | `container_ready()` thử lại với backoff (~7,5 s); nếu `ls` vẫn thấy image thì **raise** `DockerInconsistent` thay vì skip. Máy thật sự không có image vẫn được skip, nên clone mới không bị ảnh hưởng |
...
`tests/run_all.py --all` chạy lại cuối ngày **24/09/2026**, lúc máy rảnh và không sửa file nào trong lúc chạy: **842 đạt · 2 không đạt · 0 bỏ qua** (844 test), với manifest đóng băng đang có hiệu lực và trạng thái vẫn `clean` sau cả bộ test. Hai lỗi là **đúng hai lỗi cũ** — không hồi quy nào qua bốn đợt sửa của hai ngày (thang chi phí, giá cách ly, ngân sách theo chân trời, cờ $\Delta$). Cổng 1 bắt được một lỗi thật giữa đường: `spent` của `replay.rescore` lệch với chạy trực tiếp vì tôi mới tính tiền cách ly ở một trong hai đường — đúng việc test I9 sinh ra để làm.
