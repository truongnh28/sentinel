...
| **F. Đo lường và kết quả** | **69%** (5,5/8) | ✅ Lưới $\Delta$ × detector trên MockAgent · ✅ Trục $\chi$ trong đường chính: `--chi 0 0.5 1.349`, chạy ở detector mid, kèm test bắt `main()` phải gọi `sweep_chi` · ❌ $\Delta = 8$ (`deltas = (0,1,2,4)`) · 🟡 100 workflow (hiện 40) · ✅ Worst-case trên held-out: `tools/measure_heldout_transfer.py`. **Độ lợi không chuyển giao — nó đảo dấu**: Sentinel +2,7% trên dev → **−2,7%** trên held-out (draft báo 27,6%) · 🟡 Exploitability: `spikes/exploitability-full.json` có $v^{*}$, minimax mix, `worst_case_L`, `best_response_gap`, `regret` trên 4 ô $(\lambda_Q,\lambda_T)$ — nhưng trên lớp 12 attacker, chưa phải 18 · 🟡 Ablation: có C4 (bỏ belief), C8 (bỏ ngẫu nhiên hoá), NC1; thiếu rotation và benign-drift · ✅ CI bootstrap theo cụm |
...
1. **Nhóm A và D gần xong, nhóm F mới 31%.** Cỗ máy đo đã chạy được, nhưng phần lớn *số để đưa vào bài* thì chưa có. Câu trả lời cho "bài viết được bao nhiêu" là **15–20%**, không phải 83%.
...
| 4 | 12/18 attacker; best-response mới max trên $(\Delta,k)$ | Exploitability và worst-case held-out **đều đã có số**, và **cả hai ngược dấu dự phóng**. Cả hai đo trên lớp 12 luật, chưa phải 18 | Thêm 6 kịch bản rồi chạy lại cả hai; best-response brute-force trên $(k,\iota,\sigma)$ |
