...
| **B. Tấn công và ground truth** | **71%** (5/7) | ✅ Sinh $(\iota,\sigma)$ có lọc nổ sớm · ✅ Payload đóng băng và bộ sinh · 🟡 Thư viện attacker kịch bản (12/18) · 🟡 Attacker best-response: `runner.best_response_gap` đo được lợi khi quan sát chính sách và đi vào cấu hình yếu nhất, nhưng max trên $(\Delta, k)$ chứ chưa tối ưu tự do trên $(k,\iota,\sigma)$ · ❌ Marker duy nhất mỗi lần chạy (`core.py` giữ hằng `"raw_write"`) · ✅ Đo $\Delta$ thực trên 10 repo SWE-bench · ✅ Validate harm trên tập con: 15 instance chọn có seed, test ẩn **viết tay**, 4 chế độ × 15 = 60 phán quyết |
...
| 4 | 12/18 attacker; best-response mới max trên $(\Delta,k)$ | Exploitability **đã có số** nhưng trên lớp 12 và **ngược dấu** so với dự phóng; worst-case trên held-out vẫn chưa ai đo | Thêm 6 kịch bản; best-response brute-force trên $(k,\iota,\sigma)$, tái dùng `tools/evaluate_exploitability.py`; chạy nó trên `attackers.held_out()` |
