...
| **Kiểm thử** | `tests/run_all.py --all` cuối ngày **24/09**: **829 đạt · 2 không đạt · 0 bỏ qua** (831 test). Cổng 1 **611/611**, không còn test container nào bị bỏ qua (issue #9 đã sửa). Cổng 2 đỏ 203/205 (hai lỗi cũ), cổng 3 xanh 15/15 ([§3.5](#35-kiểm-thử)). |
...
**8 hoàn thành · 1 làm theo hướng khác · 2 một phần · 0 chưa bắt đầu** (11 stage, 0–10). Rà soát ngày 22/09 bằng cách đọc mã và chạy lại toàn bộ test; cập nhật 23/09 sau đợt đo W1–W6, và **24/09 sau khi sửa thang chi phí, giá cách ly, ngân sách theo chân trời, rồi rà lại toàn repo** ([runbook](docs/AuditGame-SE_Parameter_Measurement_Runbook.md)). Đợt rà cuối ngày 24/09 đưa **Stage 7 lên hoàn thành** (hai ví dụ tay thành test) và đổi *nội dung* của Stage 2, 3, 6 và 9 — chi tiết trong cột Issue.
...
- **Việc mở** — một bước của thủ tục chưa làm. Còn đúng hai: Stage 8 (`freeze.write()`, hẹn **29/09**) và bước 8 của Stage 9 (hằng $c$, chờ **quyết định T1** về mô hình quan sát).
...
**17 hoàn thành · 1 tiền đề hỏng** (18 mục; mục duy nhất chưa xong là hằng $c$, chờ quyết định T1 chứ không chờ phép đo). Cập nhật 24/09; $\eta_Q$ vẫn tính là hoàn thành nhưng **vào bài bằng quét (M2)** chứ không chốt một số, xem [Parameter Measurement Runbook](docs/AuditGame-SE_Parameter_Measurement_Runbook.md).
...
| $c$ | 🔴 **không đo được** | Tiền đề Định lý 4 hỏng: posterior đứng yên khi $B$ đi từ 2 đến 200 ⇒ ngân sách mua **hành động**, không mua **thông tin**. Chờ quyết định T1 về mô hình quan sát, không chờ một phép đo |
...
`tests/run_all.py --all` chạy lại cuối ngày **24/09/2026**, lúc máy rảnh và không sửa file nào trong lúc chạy: **829 đạt · 2 không đạt · 0 bỏ qua** (831 test). Hai lỗi là **đúng hai lỗi cũ** — không hồi quy nào qua bốn đợt sửa của hai ngày (thang chi phí, giá cách ly, ngân sách theo chân trời, cờ $\Delta$). Cổng 1 bắt được một lỗi thật giữa đường: `spent` của `replay.rescore` lệch với chạy trực tiếp vì tôi mới tính tiền cách ly ở một trong hai đường — đúng việc test I9 sinh ra để làm.

| Cổng | Kết quả |
|---|---|
| Gate 1 — Integrity | **611/611**, **0 bỏ qua**. Issue #9 đã sửa: 13 test container từng bị bỏ qua vì `docker image inspect` trên Docker Desktop trả "No such image" theo từng đợt, chứ **không phải** do CPU tải nặng như chẩn đoán cũ |
