...
| **Khung bài đã chốt** | **A′ — "bản đồ chế độ"** (chốt 18/09), nay có một **kết quả dương** đi kèm: sau khi sửa $\lambda_Q$ (đang rẻ 5,5×) và vá hai cơ chế thiếu của `Sentinel`, **SSG-G guarded tốt nhất trên cả hai trục** — $L = 0{,}988$ và regret $= 0{,}0119$, đều thấp nhất trong 8 chính sách. Điều kiện: $\lambda_T \in [0{,}26;\ 0{,}57]$, và 0,5 nằm gần mép trên. |
...
| **Rủi ro lớn nhất** | (1) Ô $\Delta = 4$ của cổng v2 vẫn đỏ — permutation test xác nhận độc lập là **tách được thật** ($p = 0{,}010$). (2) Agent thật làm theo payload **0/7** và detector thật chỉ đạt $d' \le 0{,}50$ so với $d'^{*} = 2{,}85$ cần có ⇒ mọi số harm là harm dưới mô hình tiếp nhận của MockAgent. (3) Lưới chính vẫn 40 workflow, chưa có $\Delta = 8$, chưa chạy 3 detector × 3 $\chi$ đầy đủ. (4) $\lambda_T = 0{,}5$ nằm gần mép trên vùng SSG-G thắng; khai 0,6 là B1 thắng. |
| **Kiểm thử** | `tests/run_all.py --all` ngày **23/09**: **795 đạt · 2 không đạt · 13 bỏ qua** (810 test) — đúng hai lỗi cũ, không hồi quy. Cổng 1 *UNKNOWN* (577/590, 13 test cần Docker), cổng 2 đỏ 203/205, cổng 3 xanh 15/15 (chi tiết ở [§3.5](#35-kiểm-thử)). |
