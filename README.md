...
1. **88% là công đã bỏ ra, không phải bài đã viết.** Cỗ máy đo nay chạy đúng thang và đúng giá, nhưng chính vì thế mà *số để đưa vào bài* vừa xấu đi: chỉ 2/12 ô ở $H=8$ vượt ngưỡng 15% và **0/18 ô** ở $H=12$. Câu trả lời cho "bài viết được bao nhiêu" là **25–30%**, không phải 88%.
...
| **Ô vượt ngưỡng có nằm trong vùng cổng chứng nhận không** | — | **Không — giao nhau bằng rỗng.** Cổng AUC v2 chứng nhận $\Delta \in \{0, 2\}$ (0,519 / 0,529, 20/20 split) và **từ chối $\Delta = 4$** (0,587, 1/20; permutation test xác nhận tách được thật, $p = 0{,}010$). Hai ô duy nhất vượt 15% đều ở **$\Delta = 4$**. Trong vùng đã chứng nhận, tốt nhất cả bảng là **+10,7%** (SSG-full, strong, $\Delta=2$) | Ở $\Delta = 4$ payload **phân biệt được bằng đặc trưng bề mặt**, nên ô đó đo một phần là "payload lộ" chứ không thuần "phân bổ audit dưới điều kiện ẩn". Đây là lý do mạnh nhất cho **khung B** mà tiền đăng ký [cong-v2-chot](docs/preregistration/cong-v2-chot.md) đã nêu |
