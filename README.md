...
| | |
|---|---|
| **Đang làm** | Nước rút nộp **FSE 2027 Research Papers**, hạn **02/10/2026 AoE** (trưa 03/10 giờ Việt Nam). Hôm nay là **ngày 6/14**, còn **8 ngày**. |
| **Khung bài đã chốt** | **A′ — "bản đồ chế độ"** (chốt 18/09). **24/09 đổi người thắng:** lưới chính chưa bao giờ gọi `costs.install()` nên chạy trên bảng placeholder, và đường cách ly hệ quả-của-audit **không bị trừ ngân sách**. Sửa cả hai thì khoảng cách dẫn của `Sentinel` trước B1 ở $\Delta = 4$ tụt **+43,4% → +3,5%** — nó được mua bằng cách ly miễn phí. Cái **sống sót** là họ SSG: `SSG-full receding` **+17,8%** (mid và strong, $\Delta = 4$), và SSG-G vẫn tốt nhất trên cả hai trục ($L = 0{,}988$, regret $= 0{,}0119$) vì nó **không bao giờ cách ly** nên không có gì để mất. |
| **Đang ở bước** | Stage 9 vừa chạy lại **đúng thang và đúng giá** (24/09) — và kết quả âm, xem hai dòng dưới. Stage 4 (chứng nhận cổng v2) và Stage 8 (đóng băng, 29/09) còn lại. |
| **Mốc kế tiếp** | **Trước 29/09:** chốt issue [#13](#34-issue-cần-xử-lý) — luận điểm nào còn phát biểu được sau khi `Sentinel` hoà B1. Đây là quyết định khung bài, không phải phép đo. Kèm theo: bảng chứng nhận cổng v2 chạy một lần (giữ A′ hay lùi **khung B**), rồi `freeze.write()`. |
...
| **Rủi ro lớn nhất** | (1) **Chỉ 2/12 ô vượt ngưỡng 15%** của chính dự án, cả hai là `SSG-full receding` ở $\Delta = 4$. `Sentinel` coi như hoà B1 (+1,7% … +4,6%) ⇒ bài phải đổi chính sách chủ đạo, hoặc phát biểu lại luận điểm. (2) Agent thật làm theo payload **0/7** và detector thật chỉ đạt $d' \le 0{,}50$ so với $d'^{*} = 2{,}85$ cần có ⇒ mọi số harm là harm dưới mô hình tiếp nhận của MockAgent. (3) Ô $\Delta = 4$ của cổng v2 vẫn đỏ — permutation test xác nhận độc lập là **tách được thật** ($p = 0{,}010$). (4) **Kết quả không bền theo chân trời**: cùng $\Delta = 4$, SSG-full +17,8% ở $H=8$ nhưng +4,0% ở $H=12$, nơi `B2 uniform random` thắng 5/18 ô. (5) Lưới chính vẫn 40 workflow. (6) $\lambda_T = 0{,}5$ nằm gần mép trên vùng SSG-G thắng; khai 0,6 là B1 thắng. |
| **Kiểm thử** | `tests/run_all.py --all` ngày **24/09**: **795 đạt · 2 không đạt · 13 bỏ qua** (810 test) — đúng hai lỗi cũ, không hồi quy. Cổng 1 *UNKNOWN* (577/590, 13 test cần Docker), cổng 2 đỏ 203/205, cổng 3 xanh 15/15 (chi tiết ở [§3.5](#35-kiểm-thử)). |
...
| **B. Tấn công và ground truth** | **93%** (6,5/7) | ✅ Sinh $(\iota,\sigma)$ có lọc nổ sớm · ✅ Payload đóng băng và bộ sinh · ✅ Thư viện attacker kịch bản **18/18** (6 luật mới đọc workflow; cổng 2 bác 2 bản đầu vì trùng hành vi luật cũ) · 🟡 Attacker best-response: `runner.best_response_gap` đo được lợi khi quan sát chính sách và đi vào cấu hình yếu nhất, nhưng max trên $(\Delta, k)$ chứ chưa tối ưu tự do trên $(k,\iota,\sigma)$ · ✅ Marker duy nhất mỗi lần chạy: `build.marker_for` băm (repo, workflow, carrier, $\iota$, $\sigma$, $\Delta$) — 16/16 cấu hình ra 16 marker khác nhau, tất định để replay được · ✅ Đo $\Delta$ thực trên 10 repo SWE-bench · ✅ Validate harm trên tập con: 15 instance chọn có seed, test ẩn **viết tay**, 4 chế độ × 15 = 60 phán quyết |
| **C. Corpus lành tính và cổng** | **80%** (4/5) | ✅ Corpus từ item SWE-bench thật · ✅ Ghép theo 5 đặc trưng bề mặt · 🟡 Cổng AUC v2, 20 split ($\Delta = 4$ còn đỏ) · ✅ Permutation test (200 hoán vị): $\Delta=4$ **tách được** ($p = 0{,}010$) — xác nhận độc lập ô đỏ; $\Delta \in \{0,2\}$ **không** ($p = 0{,}28$ / $0{,}25$) · 🟡 $\beta_k$ đo trên 10 repo thật; skill và queue chuyển sang quét |
...
| **E. Chính sách và lý thuyết** | **88%** (7/8) | ✅ B1–B6 · ✅ Minimax LP · ✅ Các biến thể SSG · ✅ Belief tính chính xác · ✅ Thư viện 28 policy (`policies_library.py`: 8 stage-weighted · 12 belief-threshold · 8 carrier-rotation; B1 nằm **trong** lớp) · ❌ 240 game nhỏ giải chính xác · ✅ $\rho$ đo trên **không gian đúng** (phân phối hành động *theo từng task*, TV trung bình theo $H$): **0,5078** · ✅ $\zeta$ |
| **F. Đo lường và kết quả** | **88%** (7/8) | ✅ Lưới $\Delta$ × detector trên MockAgent · ✅ Trục $\chi$ trong đường chính: `--chi 0 0.5 1.349`, chạy ở detector mid, kèm test bắt `main()` phải gọi `sweep_chi` · ✅ $\Delta = 8$ **đã chạy** (24/09, `--deltas`, $H = 12$) — và kết quả là **âm**: không ô nào vượt 15%, `B2 uniform random` thắng 5/18 ô · 🟡 100 workflow (hiện 40) · ✅ Worst-case trên held-out: ba thống kê kèm CI95 lấy mẫu theo workflow; thống kê của draft **bão hoà**, thống kê thay thế cho thấy chỉ `Sentinel` xuống cấp có ý nghĩa · ✅ Exploitability trên **8 chính sách** kèm $v^{*}$ và minimax mix: **SSG-G 0,0119** thấp hơn B1 0,0179 · 🟡 Ablation: có C4 (bỏ belief), C8 (bỏ ngẫu nhiên hoá), NC1; thiếu rotation và benign-drift · ✅ CI bootstrap theo cụm |
| **G. Tính toàn vẹn** | **90%** (4,5/5) | ✅ Ba cổng kiểm thử, 810 test · ✅ Tài liệu tiền đăng ký · 🟡 Digest rời (cổng v2, payload, prompt) · ✅ Manifest đóng băng toàn phần (`freeze.py`: digest mã nguồn + hằng đọc từ module sống + hai registry + bảng sinh) · ✅ Harness từ chối policy chưa đóng băng (`freeze.require_frozen`, gọi trong `experiment.py`) |
...
1. **87% là công đã bỏ ra, không phải bài đã viết.** Cỗ máy đo nay chạy đúng thang và đúng giá, nhưng chính vì thế mà *số để đưa vào bài* vừa xấu đi: chỉ 2/12 ô ở $H=8$ vượt ngưỡng 15% và **0/18 ô** ở $H=12$. Câu trả lời cho "bài viết được bao nhiêu" là **25–30%**, không phải 87%.
2. **Nút thắt đã chuyển, và nay là một quyết định chứ không phải một phép đo.** Thư viện 28 policy, manifest đóng băng, 18/18 attacker, $\Delta = 8$ — đều xong. Cái chặn bây giờ là **luận điểm nào còn phát biểu được**: sau khi tính đúng giá cách ly và chạy đúng thang, `Sentinel` hoà B1, chỉ họ SSG còn khoảng cách, và khoảng cách đó **không bền theo chân trời**. Xem issue #13 và #15 ở [§3.4](#34-issue-cần-xử-lý).
...
| 9 | Chạy lưới đầy đủ; worst-case trên held-out; exploitability; crossover; CI bootstrap theo cụm repo; hằng $c$ | Bảng kết quả trên lưới $(\Delta,\chi)$ | Draft (Bảng 2, $\Delta > 2$, $\chi = 1{,}34$, held-out): B1 0,412 → Sentinel 0,272 (−34,1%, CI [−42,6%; −25,1%], vượt ngưỡng 15%); held-out 27,6%; exploitability 0,34 → 0,09; cách ly nhầm 3,2% → 5,1%; crossover 2,1; RQ4 21,4%–39,8%, lớn nhất ở detector yếu; quét $\eta_Q$: 34,1% → 44,0% | ✅ $\Delta = 8$ · ✅ trục $\chi$ · ✅ worst-case held-out · ✅ exploitability trên 8 chính sách. **Đo xong thì số không đạt:** 2/12 ô ở $H=8$ và **0/18 ô** ở $H=12$ vượt ngưỡng 15%. Còn: $N = 100$ · ablation (rotation, benign-drift) · **W6** hằng $c$ |
...
| 3 | 🟡 Một phần | Sinh $(\iota,\sigma)$ có lọc nổ sớm; **18 attacker kịch bản** (6 luật đọc workflow thêm 23/09); chia held-out bằng hash; **marker duy nhất mỗi cấu hình** (`build.marker_for`); **23/09: đo $\Delta$ thực trên 10 repo SWE-bench** (SZZ-lite) | $\Delta$ theo task: trung vị **0** ở 8/10 repo, django **4,5**; chỉ **19%** instance có $\Delta \ge 2$. $\Delta$ theo commit: trung vị 14–194, p75 tới 935 | Số dogfood cũ (trung vị 3) đo trên repo đề tài, **không** phải quần thể mục tiêu; đơn vị task phụ thuộc mật độ lấy mẫu instance nên phải báo theo repo. Vùng $\Delta \ge 2$ chỉ phủ ~1/5 workload ⇒ ranh giới vùng nên phát biểu theo tỉ lệ workload từng repo. Còn lại: held-out **9/18** (theo hash, không vẽ lại để khớp 7/18 của draft); best-response mới là max trên $(\Delta,k)$ (`runner.best_response_gap`), chưa tối ưu tự do trên $(k,\iota,\sigma)$. ✅ $\Delta = 8$ đã chạy 24/09 ở $H = 12$. Harm **đã validate trên tập con**: 15 instance, test ẩn viết tay, khớp 30/30 trong miền bộ mô phỏng biểu diễn được và 0/30 ngoài miền đó |
...
| $\rho$ | 0,07 | M1 | ✅ Hoàn thành | **0,5078** trên phân phối hành động theo từng task (cũ: 1,108 khi gộp cả episode). B1 được phủ **chính xác** (khoảng cách 0,0) | Cận Mệnh đề 6 $H\rho\,\mathrm{range}(L) = 4{,}06$ ⇒ **rỗng**. Đường bão hoà phẳng từ **4 thành viên** — nhưng vì B6 two-stage cách thành viên gần nhất 0,5078 và không ai gần hơn: đó là **lỗ hổng** của lớp, không phải mật độ. Con số 28 chưa được biện hộ |
...
|---|---|---|---|
| **Độ lợi so với B1** | −34,1% (Sentinel, $\Delta > 2$) | **`Sentinel` +3,5%** ($\Delta = 4$, mid) — coi như hoà. Cái đạt được là **`SSG-full receding` +17,8%** (mid và strong, $\Delta = 4$); SSG-G +10,9…11,6% | 40 workflow × 3 seed × 3 detector, thang USD, cách ly **có tính tiền**. Chỉ **2/12 ô** vượt ngưỡng 15% của dự án. Con số +43,4% công bố trước đó chạy trên bảng placeholder **và** cách ly miễn phí — xem hai dòng dưới. Nguồn: `spikes/grid-usd-scale.txt` |
...
| **Kết quả có bền theo chân trời không** | — | **Không.** Cùng $\Delta = 4$, detector mid: `SSG-full receding` **+17,8% ở $H=8$** nhưng **+4,0% ở $H=12$**. Ở $H = 12$ **không ô nào** trong 18 vượt 15%, và **`B2 uniform random` thắng 5/18 ô**, gồm toàn bộ $\Delta = 6$ và $\Delta = 8$ | Giới hạn phải khai: `make_corpus(n, H, seed)` sinh workflow khác nhau theo $H$, nên phép so này **lẫn chân trời với corpus** — chưa tách được. Ở $\Delta = 8$ chỉ **22/40** workflow khả dụng nên mẫu số cũng co lại. `spikes/grid-usd-H12-delta8.txt` |
...
|---|---|---|---|
| 1 | Ô $\Delta = 4$ của cổng v2 đỏ; test *matched* mới đỏ | Quyết định khung bài ở ngày 6. **Permutation test xác nhận độc lập**: $\Delta=4$ tách được thật ($p = 0{,}010$), không phải giả định của cận tham số | Chốt trước: chứng nhận $\Delta \in \{0,2\}$ hay lùi khung B. Hai phép kiểm **bất đồng** ở $\Delta \in \{0,2\}$ — cận Hanley–McNeil nói vượt trần, hoán vị nói bình thường ($p = 0{,}28$/$0{,}25$) ⇒ cận tham số đang **quá bảo thủ** ở đây |
...
| 3 | ✅ **Dựng xong** `policies_library.py` (28) — nhưng $\rho$ nói **28 chưa được biện hộ** | Đường bão hoà phẳng từ 4 thành viên vì B6 two-stage cách thành viên gần nhất 0,5078 và không ai gần hơn — đó là **lỗ hổng**, không phải mật độ | Thêm một họ phủ được hành vi two-stage; thêm thành viên cho ba họ hiện có **không** giúp |
| 4 | ✅ **18/18 attacker**; best-response vẫn chỉ max trên $(\Delta,k)$ | Exploitability **khớp dự phóng** sau khi đo đúng đối tượng (SSG-G 0,0119 < B1 0,0179). Held-out: thống kê của draft bão hoà, đã thay bằng hai thống kê đọc được | Còn best-response brute-force trên $(k,\iota,\sigma)$ |
...
| 13 | 🔴 **Luận điểm chính không còn vượt ngưỡng 15%** sau khi cách ly bị tính tiền: `Sentinel` +3,5%, chỉ `SSG-full receding` đạt +17,8% ở 2/12 ô | Bài không thể phát biểu "Sentinel giảm harm 34%" nữa | Ba lựa chọn: đổi chính sách chủ đạo sang họ SSG · phát biểu lại luận điểm theo **vùng** ($\Delta \ge 2$, detector mid/strong) · hoặc báo cả hai chế độ giá cách ly và để $\eta_Q$ là trục. Đây là quyết định khung bài, phải chốt trước 29/09 |
...
| 15 | 🔴 **Kết quả không bền theo chân trời** — SSG-full +17,8% ($H=8$) so với +4,0% ($H=12$) ở cùng $\Delta = 4$; ở $H=12$ `B2 uniform random` thắng 5/18 ô | Nếu không tách được thì luận điểm chỉ phát biểu được cho **một** chân trời, và phải nói ra | Tách chân trời khỏi corpus: sinh corpus $H=12$ rồi **cắt** xuống $H=8$ thay vì sinh lại, để hai lưới dùng chung workflow |
...
`tests/run_all.py --all` chạy lại cuối ngày **24/09/2026**: **795 đạt · 2 không đạt · 13 bỏ qua** (810 test). Hai lỗi là **đúng hai lỗi cũ** — không hồi quy nào qua bốn đợt sửa của hai ngày (thang chi phí, giá cách ly, ngân sách theo chân trời, cờ $\Delta$). Cổng 1 bắt được một lỗi thật giữa đường: `spent` của `replay.rescore` lệch với chạy trực tiếp vì tôi mới tính tiền cách ly ở một trong hai đường — đúng việc test I9 sinh ra để làm.

| Cổng | Kết quả |
|---|---|
| Gate 1 — Integrity | 577/590 (+39 test mới: nguồn gốc chi phí, 4 checkpoint, trục $\chi$, marker, thư viện 28 policy, manifest đóng băng), 13 bỏ qua nên vẫn tính là *UNKNOWN* |
